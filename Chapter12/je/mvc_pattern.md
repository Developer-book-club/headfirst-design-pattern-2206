...
- View
    ...
    - 컴포지트 패턴 사용
        ...
        - 최상위 뷰 구성 요소에게만 화면 갱신 요청을 하면 됨.
    - 컨트롤러와 함께 전략패턴 사용
        - 뷰 객체를 여러 전략을 써서 설정가능
        - 뷰는 겉모습만 신경쓰고, 인터페이스 행동 결정은 컨트롤러가
        - 뷰를 모델로부터 분리하는데 도움
            
            (모델에게 요청을 처리 하는 역할을 컨트롤러가 하기때문)
...
- 특징
    - 사용자는  뷰에만 접속가능
        - 버튼을 누르는 등의 행동을 하면 뷰가 컨트롤러에게 행동을 전달하고 컨트롤러는 상황에 맞게 작업 처리
    - 컨트롤러가 모델에게 상태를 변경하라고 요청
        - 행동을 받고 해석 후 모델 조작 방법을 결정해줌
            
            (컨트롤러가 뷰를 변경해 달라고 요청)
            
    - 상태가 변경되면 모델이 뷰에게 알림
        - 모델에 있는 데이터,상태 등이 변경되면 뷰에게 알림
    - 뷰가 모델에게 상태를 요청
...
- 컨트롤러
    - ControllerInterface
        ...
        뷰에서 쓰이는 전략 → 뷰에 넣을 전략 객체의 인터페이스 필요
